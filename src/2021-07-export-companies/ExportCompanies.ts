...
(async () => {
  const columns = 'id|category|subcategories|company_name|company_address|creation_date|contact_agreement|company_siret|company_postal_code|status|company_id|tags|company_country'
    .split('|')
  const readStream = fs.createReadStream('/Users/alexandreac/Desktop/reports.txt')
    .pipe(csvParser({
      headers: columns,
      separator: '|',
    }))
  ...
  for await (const chunk of readStream) {
    ...
    if (i > 1 && chunk.company_siret) {
      const x = await fetch('http://localhost:9000/api/companies/search/' + chunk.company_siret, {
        ...
        'method': 'GET',
        'mode': 'cors'
      } as any).then(_ => _.json()).catch(err => console.error('HTTP MISS ', chunk, err))
      // console.log(x, chunk)
      try {
